...
- **未跟踪文件**: 4个（aitoearn-run ×2, team-deep-check ×2）
- **结论**: Git 工作流正常，无分叉
...
- **状态**: 🔴 下线约 **14 天**
- **表现**: curl 返回 `{"detail":"Not Found"}`（HTTP 200 but FastAPI 404）
- **原因**: Render 免费实例被暂停/删除，未收到流量触发唤醒
- **影响**: 鸠摩罗什Bot无法服务用户
...
- **jiumoluoshi-bot.onrender.com**: FastAPI 404，服务不可用
- **aitoearn.onrender.com**: 完全不可达
- **影响**: 核心业务完全停摆
- **建议**:
  1. **田太平需登录 Render Dashboard 重建服务**
  2. 检查 Render 账号状态/账单
  3. 确认免费实例是否已过期
...
| 时间 | 事件 |
|------|------|
| 08:00 | aitoearn 扫描正常，TikTok 阻塞 |
| 09:00 | coordinator 报告 |
| 10:00 | coordinator 报告 |
| 11:00 | deep-check: Render 双下线确认 |
| 11:17 | aitoearn 扫描正常，TikTok 阻塞 |
| 12:00 | coordinator 报告（本次） |
